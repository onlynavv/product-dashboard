...
import {validateFormSchema} from './AddProducts'
...
const EditForm = () => {
    ...
    useEffect(() => {
        fetch(`https://6166c4d713aa1d00170a66f3.mockapi.io/products/${id}`,{method:"GET"})
        .then((data)=> data.json())
    ...
    return (
        ...
        product ? <UpdateProduct product={product} /> : <h1>Loading...</h1>
...
const UpdateProduct = ({product}) => {
    ...
    const {handleBlur, handleChange, handleSubmit, errors, values, touched} = useFormik(
        {
            initialValues:{name:product.name,image:product.image,price:product.price,stock:product.stock,status:product.status,rating:product.rating},
    ...
    const handleEdit = (values) => {
        fetch(`https://6166c4d713aa1d00170a66f3.mockapi.io/products/${product.id}`, {
        method:'PUT',
            ...
            <article className="container add-product-wrapper">
              <Card className="form-card">
                ...
                <form className="form-wrapper" onSubmit={handleSubmit}>
                  ...
                  <Button className="submitBtn" variant="contained" size="medium"  type="submit">Edit Product</Button>
                </form>
                </CardContent>
                </Card>
            </article>
